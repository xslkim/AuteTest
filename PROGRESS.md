...
|----|------|------|------|------|--------|------|
| T0.1 | 仓库骨架 | done | 2026-05-01T12:05:00Z | 2026-05-01T12:35:00Z | 4734617 | 实现 commit；start=e033779 |
...
- 备注：TASKS 写「8 个子命令」与 PRD §7 的 9 个 CLI 不一致；按 PRD 注册 `build` / `compile` / `tts` / `visuals` / `render` / `preview` / `cache` / `doctor` / `init`。`remotion.config.ts` 使用 `@remotion/cli/config` 的 `Config`（devDependency `@remotion/cli`）。验收环境 apt 提供 Node v18；PRD 要求 Node ≥ 20 供后续 `doctor` / CI 校验。
...
- 模糊点：TASKS.md T0.1 写 commander 注册「8 个子命令」，PRD §7 列出 `build` / `compile` / `tts` / `visuals` / `render` / `preview` / `cache` / `doctor` / `init` 共 9 个。
- 选择方案：按 PRD 注册全部 9 个子命令 stub。
- 备选方案：仅注册 8 个会遗漏 `init` 或 `doctor`，与帮助/合同不一致。
- 影响范围：仅 CLI stub；后续各命令实现不受影响。

...
- **next_action**: `实现并验收 T9.1 — 统一 vitest 用例文件名 + cache CLI 单测`
- **completed**: `36 / 40`
- **blockers**: `0`
...
| T9.1 | 单测补全 | in_progress | 2026-05-03T12:00:00Z | — | — | — |
| T9.2 | E2E 测试 | pending | — | — | — | — |
...
- artifacts: `src/cli/init.ts` / `templates/starter/*` / `bin/autovideo.ts` / `tests/init-cli.test.ts`
- 备注：`doctor-cli` 单测在 Node 低于 v20 的环境下允许「Node 版本」为 FAIL（其余项仍须 PASS）
...
- artifacts: `src/render/root-render.ts` / `tests/root-render.test.ts` / `tests/__snapshots__/root-render.test.ts.snap`
- 备注：导入路径 `./public/script.json` 与 `../../remotion/VideoComposition` 相对 build out dir；生成前校验每块含 `timing`

...
- **active_task**: `T0.2`
- **last_updated**: `2026-05-01T05:12:40Z`
...
|----|------|------|------|------|--------|------|
| T0.1 | 仓库骨架 | done | 2026-05-01T05:10:34Z | 2026-05-01T05:12:40Z | 908c506 | — |
...
### T0.1 — 仓库骨架 @ 908c506
- acceptance: npm install ✓；npx tsx bin/autovideo.ts --help 列出全部子命令 ✓；compile stub 退出码 1 + "not implemented" ✓
- artifacts: `package.json`、`package-lock.json`、`tsconfig.json`、`remotion.config.ts`、`bin/autovideo.ts`、`.gitignore`、`PRD.md`（§13.1 增补 `@remotion/cli`）
- 备注：验收环境中 apt 初始无 Node；已安装 nodejs/npm（v18）以完成验收。
...
### T0.1 | §13.1 Node 依赖列表 | remotion.config 需要 `@remotion/cli`
- PRD 原描述：`dependencies` 中仅 `@remotion/bundler`、`@remotion/renderer`、`@remotion/google-fonts` 等，未列出 `@remotion/cli`。
- 实际实现：`remotion.config.ts` 使用 `@remotion/cli/config` 的 `Config`，故在 `package.json` 中增加 `@remotion/cli`，并在 PRD §13.1 同步列出。
- 原因：`Config.setKeyframeInterval` 等 API 由 CLI 包导出；未安装时模块无法解析。
- PRD 是否同步更新：是（908c506）

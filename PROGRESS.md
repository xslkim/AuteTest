...
|----|------|------|------|------|--------|------|
| T0.1 | 仓库骨架 | done | 2026-05-01T12:00:00Z | 2026-05-01T12:35:00Z | 7e9b067 | — |
...
### T0.1 — 仓库骨架 @ 7e9b067
- acceptance: `npm install` 成功 → ✓；`npx tsx bin/autovideo.ts --help` 显示所有子命令（含 `cache stats` / `cache clean`）→ ✓；`npx tsx bin/autovideo.ts compile foo.json` 退出码 1 + `not implemented` → ✓
- artifacts: `package.json`、`package-lock.json`、`tsconfig.json`、`remotion.config.ts`、`bin/autovideo.ts`、`.gitignore`（追加构建与缓存忽略项）
- 备注：环境通过 apt 安装 Node 18 完成验收；PRD 要求 Node ≥ 20，后续 CI/本机建议升级到 20+。

...
|----|------|------|------|------|--------|------|
| T0.1 | 仓库骨架 | done | 2026-05-01T05:11:07Z | 2026-05-01T05:25:00Z | 64563493d2aab83ef07ffde4f9d0cd920752a5d8 | — |
...
### T0.1 — 仓库骨架 @ 64563493d2aab83ef07ffde4f9d0cd920752a5d8
...
- 备注：验收环境通过 apt 安装 Node 18；PRD `doctor` 要求 Node ≥ 20，后续 CI / 本机应按文档对齐。
...
- 模糊点：PRD §13.1 `package.json` 依赖列表未列出 `@remotion/cli`，但 `remotion.config.ts` 需 `import { Config } from "@remotion/cli/config"`。
- 选择方案：在 `dependencies` 中增加 `"@remotion/cli": "^4.0.0"`，与其它 Remotion 包 major 对齐。
- 备选方案：仅用环境变量或手写 webpack 配置替代 Remotion Config API — 复杂度高且偏离 PRD §6.4。
- 影响范围：仅 `package.json` 依赖声明；不影响运行时契约。

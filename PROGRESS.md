...
|----|------|------|------|------|--------|------|
| T0.1 | 仓库骨架 | done | 2026-05-01T12:10:00Z | 2026-05-01T12:35:00Z | `25d9592` | — |
...
- artifacts: `package.json`、`package-lock.json`、`tsconfig.json`、`remotion.config.ts`、`bin/autovideo.ts`、`.gitignore`（增补构建忽略项）
- 备注：`package.json` 增加 `@remotion/cli` 以使 `remotion.config.ts` 中的 `Config` 类型与 CLI 配置一致（PRD §13.1 所列依赖外）。
...
- 模糊点：PRD §13.1 `package.json` 依赖清单未列出 `@remotion/cli`，但 TASKS.md 要求根目录 `remotion.config.ts` 使用 `Config.setKeyframeInterval` / `Config.setVideoImageFormat`（官方文档中 `Config` 出自 `@remotion/cli/config`）。
- 选择方案：将 `@remotion/cli` 以 `^4.0.0` 加入 `dependencies`，与其他 `@remotion/*` 主版本对齐。
- 备选方案：假定 `remotion` 包 re-export `Config`——未采纳：API 边界与版本耦合不清晰。
- 影响范围：仅依赖体积；CLI / Studio 与 bundler 使用同源 `@remotion/cli`。

...
5. [x] 已确认 `git log -1` 的 hash 与下表中最近一个 `done` 任务的 commit 一致（下表 T0.1 commit 列与 `git log` 核对在会话内完成；后续恢复时请按此步骤比对）
...
|----|------|------|------|------|--------|------|
| T0.1 | 仓库骨架 | done | 2026-05-01T12:30:00Z | 2026-05-01T12:50:00Z | 1f9d3dd | — |
...
- artifacts: `package.json`、`package-lock.json`、`tsconfig.json`、`remotion.config.ts`、`bin/autovideo.ts`、`.gitignore`（含 node_modules/dist 等）
- 备注：Remotion 4 无 `setKeyframeInterval`，GOP 通过 `overrideFfmpegCommand` 注入；PRD §6.4 已同步。

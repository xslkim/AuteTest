...
- `remotion.config.ts`：`Config.setVideoImageFormat('jpeg')` + `Config.overrideFfmpegCommand` 在 x264 stitch 阶段注入 `-g 1 -keyint_min 1`（等价于原 `setKeyframeInterval(1)`，Remotion 4 已移除该 API；保证 §6.4 step 6 GOP/IDR）

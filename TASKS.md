...
- `remotion.config.ts`：`Config.overrideFfmpegCommand` 在 `-c:v libx264` 后注入 `-x264-params keyint=1:min-keyint=1:scenecut=0`（等价于 §6.4 的 GOP/IDR；Remotion 4.x 已无 `setKeyframeInterval`）+ `Config.setVideoImageFormat('jpeg')`

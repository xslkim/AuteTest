...
- `remotion.config.ts`：每帧 GOP / IDR：`Config.overrideFfmpegCommand` 在 libx264 编码前插入 `-g 1 -keyint_min 1 -sc_threshold 0`（Remotion 4 无 `setKeyframeInterval`；与 PRD §6.4「或等价配置」一致）+ `Config.setVideoImageFormat('jpeg')`

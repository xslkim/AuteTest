...
- `remotion.config.ts`：`Config.overrideFfmpegCommand` 为 `pre-stitcher` 的 libx264 注入 `keyint=1`…（等价 §6.4 GOP / IDR；Remotion v4 无 `setKeyframeInterval`）+ `Config.setVideoImageFormat('jpeg')`

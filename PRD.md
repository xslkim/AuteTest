...
6. **ffmpeg concat 拼接**（在 build out dir/`output/` 下执行；`partials/B**.mp4` 是相对此 cwd 的路径）：
   ...
   - **GOP 起点对齐 IDR**：每个 partial 必须以 IDR 关键帧开头才能被流复制 concat。Remotion 4 已移除 `Config.setKeyframeInterval()`；在 `remotion.config.ts` 中用 `Config.overrideFfmpegCommand()` 在 stitcher 阶段为 x264 编码插入 `-g 1 -keyint_min 1`（或等价的「每帧 IDR」配置），保证每段 mp4 首帧为 IDR；否则少数情况会出现首帧绿屏 / 丢帧

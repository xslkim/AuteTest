...
6. **ffmpeg concat 拼接**（在 build out dir/`output/` 下执行；`partials/B**.mp4` 是相对此 cwd 的路径）：
   ...
   - **GOP 起点对齐 IDR**：每个 partial 必须以 IDR 关键帧开头才能被流复制 concat。Remotion 4 起 `@remotion/cli/config` 已无 `setKeyframeInterval`；实现上在 **render 阶段**向编码器传入等价于 **每帧关键帧**（或强制首帧 IDR）的 FFmpeg/x264 参数，保证每段 mp4 首帧为 IDR；`remotion.config.ts` 仅保留与帧导出相关的项目级设置（例如 `Config.setVideoImageFormat('jpeg')`）。否则少数情况会出现首帧绿屏 / 丢帧

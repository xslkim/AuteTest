...
6. **ffmpeg concat 拼接**（在 build out dir/`output/` 下执行；`partials/B**.mp4` 是相对此 cwd 的路径）：
   ...
   - **GOP 起点对齐 IDR**：每个 partial 必须以 IDR 关键帧开头才能被流复制 concat。Remotion 4 起 `Config.setKeyframeInterval()` 已移除；项目在 `remotion.config.ts` 中对 stitcher 阶段的 `libx264` 注入 `-g 1 -keyint_min 1 -sc_threshold 0`（通过 `Config.overrideFfmpegCommand`），使每帧均为关键帧，等价于段首 IDR，满足 `-c copy` concat；否则少数情况会出现首帧绿屏 / 丢帧
...
```json
{
  "dependencies": {
    ...
    "@remotion/cli": "^4.0.0",             // remotion.config.ts：`Config` 从 `@remotion/cli/config` 导入（Remotion 4）

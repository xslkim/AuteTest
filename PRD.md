...
6. **ffmpeg concat 拼接**（在 build out dir/`output/` 下执行；`partials/B**.mp4` 是相对此 cwd 的路径）：
   ...
   - **GOP 起点对齐 IDR**：每个 partial 必须以 IDR 关键帧开头才能被流复制 concat。Remotion 4 已无 `Config.setKeyframeInterval`；在 `remotion.config.ts` 用 `Config.overrideFfmpegCommand` 对 stitcher 阶段的 `libx264` 注入 `-g 1`（或等价手段）保证每帧关键帧，从而使每段 mp4 首帧为 IDR；否则少数情况会出现首帧绿屏 / 丢帧

...
6. **ffmpeg concat 拼接**（在 build out dir/`output/` 下执行；`partials/B**.mp4` 是相对此 cwd 的路径）：
   ...
   - **GOP 起点对齐 IDR**：每个 partial 必须以 IDR 关键帧开头才能被流复制 concat。Remotion v4 已移除 `Config.setKeyframeInterval`，在 `remotion.config.ts` 中用 `Config.overrideFfmpegCommand` 在 `pre-stitcher` + `libx264` 路径注入 `-x264-params keyint=1:min-keyint=1:scenecut=0:no-mbtree=1`（每帧 IDR）；否则少数情况会出现首帧绿屏 / 丢帧

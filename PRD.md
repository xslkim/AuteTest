...
6. **ffmpeg concat 拼接**（在 build out dir/`output/` 下执行；`partials/B**.mp4` 是相对此 cwd 的路径）：
   ...
   - **GOP 起点对齐 IDR**：每个 partial 必须以 IDR 关键帧开头才能被流复制 concat。Remotion v4 已无 `Config.setKeyframeInterval`；`remotion.config.ts` 使用 `Config.overrideFfmpegCommand` 在 libx264 stitcher 命令中注入 `-g 1 -keyint_min 1 -sc_threshold 0`，等价于 GOP=1（每帧 IDR）；否则少数情况会出现首帧绿屏 / 丢帧

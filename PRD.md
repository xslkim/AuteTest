...
6. **ffmpeg concat 拼接**（在 build out dir/`output/` 下执行；`partials/B**.mp4` 是相对此 cwd 的路径）：
   ...
   - **GOP 起点对齐 IDR**：每个 partial 必须以 IDR 关键帧开头才能被流复制 concat。Remotion v4：`remotion.config.ts` 使用 `@remotion/cli/config` 的 `Config.overrideFfmpegCommand`，在 libx264 编码参数中注入 `-g 1 -keyint_min 1`（等价于 TASKS 原定 `Config.setKeyframeInterval(1)` 的 GOP 对齐目的）；否则少数情况会出现首帧绿屏 / 丢帧

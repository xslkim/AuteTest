...
6. **ffmpeg concat 拼接**（在 build out dir/`output/` 下执行；`partials/B**.mp4` 是相对此 cwd 的路径）：
   ...
   - **GOP 起点对齐 IDR**：每个 partial 必须以 IDR 关键帧开头才能被流复制 concat。仓库根 `remotion.config.ts` 通过 `Config.overrideFfmpegCommand` 对 `libx264` 注入 `-g 1`（每帧关键帧），保证 Remotion 输出的每段 mp4 首帧都是 IDR（Remotion `Config` 未暴露 `setKeyframeInterval`，此为等价手段）；否则少数情况会出现首帧绿屏 / 丢帧
